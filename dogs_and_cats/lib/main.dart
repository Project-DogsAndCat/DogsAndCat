...
import 'core/firebase_api/firebase_api.dart';
...
Future<void> _firebaseMessagingBackgroundHandler(RemoteMessage message) async {
  ...
  FirebaseApi().showNotification(message, flutterLocalNotification, channel);
}
...
void main() async {
  ...
  // Обработка уведомлений, когда приложение свёрнуто или закрыто.
  FirebaseMessaging.onBackgroundMessage(_firebaseMessagingBackgroundHandler);

  // Приложение находится на переднем плане
  FirebaseMessaging.onMessage.listen(
    (message) {
      api.handleNavigation(message.data);
    },
  );
